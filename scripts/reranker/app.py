...
RERANKER_MODEL_PATH = os.path.join(os.path.dirname(__file__), "model")
...
class Chat(object):
    def __init__(self, RERANKER_MODEL_PATH: str = RERANKER_MODEL_PATH):
        self.reranker = ReRanker(RERANKER_MODEL_PATH)

    def fit_query_answer_reranker(self, query_docs: QADocs) -> List:
...
async def handle_post_request(docs: QADocs, credentials: HTTPAuthorizationCredentials = Security(security)):
    ...
    chat = Chat()
    try:
        results = chat.fit_query_answer_reranker(docs)
